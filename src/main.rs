...
usage:
  ...
  lisplens version | -v | --version   name, version, copyright, and license pointer
  lisplens license              full license text: lisplens (MPL-2.0) + every bundled dependency
...
/// Open-source license notices for lisplens (MPL-2.0) and every bundled Rust
/// dependency, generated by `cargo about` and embedded so the `license` command
/// works for a binary-only distribution with no accompanying files.
...
/// The full `license` output: lisplens's own MPL-2.0 license and every bundled
/// dependency's notice, embedded so it works with no accompanying files.
fn license_text() -> String {
    format!(
        "lisplens {} — open source licenses\n{}\n\n{THIRD_PARTY_LICENSES}",
        env!("CARGO_PKG_VERSION"),
        env!("CARGO_PKG_REPOSITORY"),
    )
}
...
/// `license`: lisplens's own license plus the bundled third-party notices.
fn license() -> ExitCode {
    print!("{}", license_text());
    ExitCode::SUCCESS
}
...
mod tests {
    ...
    #[test]
    fn license_bundles_mpl_and_third_party_notices() {
        let l = license_text();
        // lisplens's own license (MPL-2.0) is included.
        assert!(l.contains("Mozilla Public License Version 2.0"));
        // Generated notices, embedded — so a binary-only distribution carries the
        // dependency license texts.
        assert!(l.contains("Open Source Licenses"));
        assert!(
            l.contains("Apache License"),
            "Apache-2.0 dependency text must be bundled"
        );
        // A specific bundled crate is listed (sanity that the file was generated
        // from real metadata, not a stub).
        assert!(l.contains("bitflags"), "expected a known dependency");
    }

    #[test]
    fn third_party_file_looks_generated_not_stub() {
        // Guards against committing an empty/placeholder file: it must carry
        // several license sections and real license text.
        assert!(THIRD_PARTY_LICENSES.matches("\n## ").count() >= 5);
        assert!(THIRD_PARTY_LICENSES.contains("Permission is hereby granted")); // MIT body
        assert!(THIRD_PARTY_LICENSES.contains("Mozilla Public License")); // lisplens itself
    }
