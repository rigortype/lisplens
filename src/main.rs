...
/// lisplens's own license (MPL-2.0), embedded so the `license` command works for
/// a binary-only distribution.
const LICENSE_MPL: &str = include_str!("../LICENSE");
/// Bundled third-party dependency notices, generated by
...
/// The full `license` output: lisplens's MPL-2.0 text followed by the bundled
/// third-party notices — both embedded, so it works with no accompanying files.
fn license_text() -> String {
    format!(
        "lisplens {} — license\n{}\n\n\
================================================================================\n\
lisplens is licensed under the Mozilla Public License 2.0:\n\
================================================================================\n\n\
...
mod tests {
    ...
    fn license_bundles_mpl_and_third_party_notices() {
        let l = license_text();
        // lisplens's own license.
        assert!(l.contains("Mozilla Public License Version 2.0"));
        // The generated third-party section, embedded — so a binary-only
        // distribution carries the dependency notices.
        assert!(l.contains("Third-Party Licenses"));
        assert!(
            l.contains("Apache License"),
            "Apache-2.0 dependency text must be bundled"
        );
        // A specific bundled crate's notice is present (sanity that the file was
        // generated from real metadata, not a stub).
        assert!(l.contains("### bitflags "), "expected a known dependency");
    }

    #[test]
    fn third_party_file_looks_generated_not_stub() {
        // Guards against committing an empty/placeholder THIRD-PARTY file: it must
        // list several crates and carry real license text.
