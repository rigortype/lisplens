...
**Where.** On **`master`**, clean, at `bcc5c17` — two post-release docs commits past
the release commit `b9eb5b2` / tag **`v0.5.0`** (the 0.5.0 record + the release-prep
skill fix below). crates.io + the 5-platform GitHub Release binaries are live and
verified (see the "Released 0.5.0" bullet under **Now**). No open issues or PRs.

**Release gotcha learned — now fixed in the release-prep skill.** `cargo about`
includes lisplens *itself* in `THIRD-PARTY-LICENSES.md` (its own MPL-2.0 entry carries
the version number), so a version bump alone makes that file stale and the CI drift
guard fails the release PR. The fix — **regenerate `THIRD-PARTY-LICENSES.md`
(`cargo about generate about.hbs -o THIRD-PARTY-LICENSES.md`) in the version-bump
commit** — is now a step in `.claude/skills/lisplens-release-prep/SKILL.md` (commit
`bcc5c17`), so the next release won't hit it.

**New dev tooling (0.5.0) — license notices + audit.** `lisplens license` embeds
`THIRD-PARTY-LICENSES.md`, generated by **`cargo about`** (config: `about.toml` +
`about.hbs`, 5 targets, dedup by license text); licenses are audited by **`cargo deny
check licenses`** (config: `deny.toml`, a strict allow-list of exactly the 6 licenses in
use). CI runs both (installs them via `taiki-e/install-action`). To regenerate/audit
locally you need them installed: `cargo install cargo-about --features cli` (the `cli`
feature is required or no binary is installed) and `cargo install cargo-deny`. The
`--version` build stamp comes from `build.rs` (git rev + date, honoring
`SOURCE_DATE_EPOCH`). Keep `about.toml`'s `accepted` and `deny.toml`'s `allow` in sync.
