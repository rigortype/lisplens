...
## Handoff — resume here: 0.5.0 released (Structural diff + version/license); next is verify parinfer or new work

**Where.** On **`master`**, clean, at the release commit `b9eb5b2` (merge of PR #49,
tagged **`v0.5.0`**). crates.io + the 5-platform GitHub Release binaries are live and
verified (see the "Released 0.5.0" bullet under **Now**). No open issues or PRs.

**Release gotcha learned (add to the release-prep skill):** `cargo about` includes
lisplens *itself* in `THIRD-PARTY-LICENSES.md` (its own MPL-2.0 entry carries the
version number), so a version bump makes that file stale and the CI drift guard
fails. **Regenerate `THIRD-PARTY-LICENSES.md` (`cargo about generate about.hbs -o
THIRD-PARTY-LICENSES.md`) as part of every version bump**, in the same release commit.

**What shipped in 0.5.0 — the Structural diff arc (#39–#42, #44; PRs #43/#45/#46) plus
version/license (PRs #47/#48).** A `diff`
...
**Candidate next work (nothing blocking):** verify parinfer on real Emacs (below);
the skill's *description* triggering-optimization loop (skill-creator, not yet run); and
a `--verbose`/full-verbatim added-body option for `diff --deep` (the framing experiment's
leftover — `--deep`'s Lens is preview-only), not filed.
...
- **Released 0.5.0** (2026-07-06) — the **Structural diff** release. A `diff` command +
  MCP tool that compares two versions of a Lisp file *modulo formatting*: a
  definition-level change map (ADR-0047), an intra-definition `--deep`/`--unit` tree diff
  (ADR-0048) that also renders added/removed defs as their `expand` Lens (#44), a
  self-contained `--html` visual view (#42), and an MCP two-form primitive
  (`oldForm`/`newForm`). Verified on `cc-engine.el` emacs-30 → emacs-31. Also: a compact
  php/ruby-style `lisplens version` banner (build stamp + git revision via `build.rs`) and
  a `lisplens license` command embedding the notices of every bundled dependency —
  generated by `cargo about`, audited in CI by `cargo deny check licenses`. Tag `v0.5.0`
  (release commit `b9eb5b2`, merge PR #49); crates.io + GitHub Release (5-platform
  binaries) live and verified. 238 tests, 48 ADRs.
