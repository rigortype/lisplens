...
| [0003](0003-discarded-forms-dropped.md) | `#_` / `#;` discarded forms are dropped from the tree | low | open — proposed: opt-in `Options.keep_discarded` keeping them as `Prefixed { Discard, … }` |
| [0004](0004-phel-semicolon-in-symbol.md) | Phel reader splits a symbol at an interior `;` | low | open — proposed: `Options.line_comment_in_atom` (Phel) |
| [0005](0005-phel-pipe-anonymous-function.md) | Phel `\|(…)` anonymous-function reader macro unrecognized | medium | open — proposed: `roles.short_fn = Some('\|')` for Phel |
| [0006](0006-phel-fqn-vs-char-literal.md) | Phel PHP FQN `\Foo\Bar` mis-read as `\`-char literals | medium | open — proposed: Phel char-literal boundary guard |
