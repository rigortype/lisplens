...
| `refactor` | Semantic refactoring procedures (ADR-0032): `rename_symbol_in_file` (symbol-exact), `inline_definition_in_file` (expand a function at its call sites, safe subset), and `rewrite_in_file` (structural pattern→template "sed", ADR-0033 — matcher + structural-equality + template substituter) over `structural` + the safety pipeline |
...
`find`/`refs`/`format`/`check`/`rename`/`inline`/`rewrite`. Edit tools take a
`patch`/`spec` string (ADR-0019's JSON op-array is a future option).
