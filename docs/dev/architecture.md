...
| `refactor` | Semantic refactoring procedures (ADR-0032): `rename_symbol_in_file` (symbol-exact, whole-file) over `structural` + the safety pipeline; `inline`/`extract` are future members |
...
`find`/`refs`/`format`/`check`/`rename`. Edit tools take a `patch` string
